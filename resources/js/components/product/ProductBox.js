...
export default function ProductBox(props) {
    ...
    return (
        <div className="tw-rounded-md tw-mr-2 tw-border tw-border-gray-200 tw-bg-white">
            <div className="tw-bg-yellow-500 tw-text-blue-900 tw-rounded-t-md tw-font-bold tw-text-xs tw-w-full tw-uppercase tw-px-1 tw-py-1 tw-text-center tw-mb-2">{discount}% Discount</div>
            <div className="tw-pt-5">
                <div className="tw-flex tw-justify-center">
                    <Link to={`/product/${id}`}>
                        <img loading="lazy" className="tw-w-20 tw-transition tw-duration-500 tw-ease-in-out tw-transform hover:tw-scale-110" src={image} />
                    </Link>
                </div>
                <Link to={`/product/${id}`}>
                ...
                <div className="tw-pt-3">
                    <Link to={`/product/${id}`}>
                        <button className="hover:tw-bg-blue-900 tw-bg-yellow-500 tw-w-full tw-text-white tw-py-2 tw-rounded-b-md tw-flex tw-justify-center tw-items-center tw-transition tw-duration-500"><img src="https://img.icons8.com/material-outlined/18/ffffff/buy.png" className="tw-mr-1"/> Buy Now</button>
                    </Link>
                </div>
            </div>
        </div>
